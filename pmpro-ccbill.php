...
*/
	...
	class PMProGateway_CCBill extends PMProGateway
	{
		...
		<tr class="gateway gateway_ccbill" <?php if($gateway != "ccbill") { ?>style="display: none;"<?php } ?>>
			...
			<td>
				<p><?php _e('To fully integrate with CCBill, be sure to use the following for your Webhook URL', 'paid-memberships-pro' );?> <pre><?php echo admin_url("admin-ajax.php") . "?action=twocheckout-ins";?></pre></p>
		...
		{						
			...
			$ccbill_sub_account_number = pmpro_getOption('ccbill_sub_account_number');
			...
			$ccbill_args['clientSubacc'] = $ccbill_sub_account_number;
			$ccbill_args['salt'] = $ccbill_salt;
			...
			//HV Comment: In CCBill Test mode must be set in the CCBIll Admin!
			//We should disable/remove the Sanbox & Live dropdown and replace
			//with a message.
